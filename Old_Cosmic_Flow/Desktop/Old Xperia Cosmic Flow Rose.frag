...
// Ondas luminosas en tonos rosas y fucsias con antialiasing y velo de seda

void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    // ==========================================
    // CONFIGURACIÓN DE COLORES (Rosas y fucsias personalizados)
    // ==========================================
    // Valores RGB en escala de 0.0 a 1.0
    vec3 u_bgColor     = vec3(0.02, 0.005, 0.015);   // Color base del fondo oscuro (con sutil tinte magenta/rosado)
    vec3 u_bgGlowColor = vec3(0.15, 0.02, 0.08);     // Color del resplandor central del fondo (rosa oscuro / magenta)
    
    vec3 u_waveColorA  = vec3(0.8, 0.15, 0.5);       // Primer tono de la onda (Rosa fucsia base)
    vec3 u_waveColorB  = vec3(1.0, 0.4, 0.75);       // Segundo tono de la onda (Rosa claro / magenta brillante)
    // ==========================================
    ...
    for (float i = 1.0; i <= 4.0; i++) {
        ...
        // Mezcla de colores rosas personalizados para las ondas
        vec3 waveColor = mix(
            u_waveColorA, 
            u_waveColorB, 
            sin(i + t * 0.5) * 0.5 + 0.5
