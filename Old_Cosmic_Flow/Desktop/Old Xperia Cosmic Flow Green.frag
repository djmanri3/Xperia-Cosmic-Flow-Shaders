...
// Ondas luminosas en tonos verdes con antialiasing y velo de seda

void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    // ==========================================
    // CONFIGURACIÓN DE COLORES (Verdes personalizados)
    // ==========================================
    // Valores RGB en escala de 0.0 a 1.0
    vec3 u_bgColor     = vec3(0.01, 0.02, 0.01);     // Color base del fondo oscuro (con sutil tinte verde)
    vec3 u_bgGlowColor = vec3(0.0, 0.15, 0.08);      // Color del resplandor central del fondo (verde esmeralda)
    
    vec3 u_waveColorA  = vec3(0.1, 0.75, 0.3);       // Primer tono de la onda (Verde vibrante base)
    vec3 u_waveColorB  = vec3(0.4, 1.0, 0.55);       // Segundo tono de la onda (Verde claro brillante)
    // ==========================================
    ...
    for (float i = 1.0; i <= 4.0; i++) {
        ...
        // Mezcla de colores verdes personalizados para las ondas
        vec3 waveColor = mix(
            u_waveColorA, 
            u_waveColorB, 
            sin(i + t * 0.5) * 0.5 + 0.5
