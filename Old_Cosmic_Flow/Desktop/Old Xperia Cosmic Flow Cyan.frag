...
// Ondas luminosas con colores personalizables para el fondo y las ondas
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    vec3 u_bgColor     = vec3(0.0, 0.0, 0.0);    // Color base del fondo oscuro
    vec3 u_bgGlowColor = vec3(0.0, 0.12, 0.15);      // Color del resplandor central del fondo
    
    vec3 u_waveColorA  = vec3(0.0, 1.0, 0.9);      // Primer tono de la onda (Cian/Turquesa base)
    vec3 u_waveColorB  = vec3(0.2, 1.0, 0.95);       // Segundo tono de la onda (Cian/Turquesa brillante)
    // ==========================================
    ...
    for (float i = 1.0; i <= 4.0; i++) {
        ...
        // Mezcla de colores personalizados para las ondas
        vec3 waveColor = mix(
            u_waveColorA, 
            u_waveColorB, 
            sin(i + t * 0.5) * 0.5 + 0.5
