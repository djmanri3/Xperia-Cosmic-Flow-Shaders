...
// Ondas luminosas en tonos azules con antialiasing y velo de seda

void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    // ==========================================
    // CONFIGURACIÓN DE COLORES (Azules personalizados)
    // ==========================================
    // Valores RGB en escala de 0.0 a 1.0
    vec3 u_bgColor     = vec3(0.01, 0.01, 0.03);     // Color base del fondo oscuro (con sutil tinte azul)
    vec3 u_bgGlowColor = vec3(0.0, 0.08, 0.2);       // Color del resplandor central del fondo (azul profundo)
    
    vec3 u_waveColorA  = vec3(0.1, 0.5, 0.9);        // Primer tono de la onda (Azul vibrante base)
    vec3 u_waveColorB  = vec3(0.3, 0.85, 1.0);       // Segundo tono de la onda (Azul cian brillante)
    // ==========================================
    ...
    for (float i = 1.0; i <= 4.0; i++) {
        ...
        // Mezcla de colores azules personalizados para las ondas
        vec3 waveColor = mix(
            u_waveColorA, 
            u_waveColorB, 
            sin(i + t * 0.5) * 0.5 + 0.5
