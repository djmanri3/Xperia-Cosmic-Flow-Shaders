...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
    ...
    // =========================================================
    // CAMBIA EL COLOR DEL FONDO AQUÍ:
    vec3 colorFondo = vec3(0.05, 0.02, 0.1);
    // =========================================================

    vec3 colorOndas = vec3(0.9, 0.7, 0.23);
    float vineta = length(uv - vec2(.5, .8)) * .1;

    vec3 col = (colorFondo - vineta) + (colorOndas * f * 1.5);
